...
Continuous work — how long since you last stepped away.

Pause pattern — empty buckets appearing inside an otherwise active stretch.

Start-stop rhythm — bursts of activity separated by stalls (the shape of "write, delete, stare, repeat").

Time since last reset — how long since your last break, quest, or brain dump.
...
openai or local. Leave blank to auto-detect from whether a key is set.

local

OPENAI_API_KEY

Your OpenAI API key. Optional — the app works fully without one.
